...
- 分析前引用相关用户原文；依赖前面多轮时一并引用，合计过长时才明确标记省略。
- 分析问题时用空间框线图表达依赖、冲突、owner 和问题背后的问题，而不是把问题平铺成一串列表。
- 梳理问题集，找到更本质的需求或矛盾，判断答案是否成熟，再按清楚的决策顺序汇报。

**使用示例**

```text
/understanding-first-report 先引用相关多轮用户原文，再重新连接问题空间，
说明哪些已查清、哪些仍待确认，最后给出洞见和下一步。
```
